...
export default function ProfilePage() {
  ...
  const { user: authUser, isLoggedIn, isLoading: authLoading, getAuthToken } = useAuth();
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!isLoggedIn) {
        // Redirect to login with a redirect parameter back to profile
  ...
  const fetchProfile = async () => {
    ...
    try {
      // Get the token using the auth context
